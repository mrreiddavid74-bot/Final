...
export function deliveryFromGirth(settings: Settings, wMm: number, hMm: number, tMm = 10): { band: string; price: number } {
  ...
  // 1) settings.delivery = { baseFee, bands: [{ maxGirthCm|maxSumCm, price|surcharge, name? }] }
  if (s.delivery?.bands?.length) {
    ...
    const pick = (b: any) => ({
      ...
      price: typeof b.price === 'number' ? b.price : (typeof b.surcharge === 'number' ? s.delivery.baseFee + b.surcharge : s.delivery.baseFee || 0),
      name: b.name ?? `${Math.round((typeof b.maxGirthCm === 'number' ? b.maxGirthCm : (b.maxSumCm ?? 0)))} cm`,
    });
    const norm = bands.map(pick).sort((a: {max:number;price:number;name:string}, b: {max:number;price:number;name:string}) => a.max - b.max);
    const band = norm.find((b: {max:number;price:number;name:string}) => girthCm <= b.max) || norm.at(-1)!;
    return { band: band.name, price: band.price };
  }

  // 2) settings.deliveryBase + settings.deliveryBands (flat form)
  if (typeof s.deliveryBase === 'number' && Array.isArray(s.deliveryBands)) {
    const base = s.deliveryBase;
    const norm = s.deliveryBands.map((b:any) => ({
      max: b.maxGirthCm ?? b.maxSumCm ?? Infinity,
      price: base + (b.surcharge ?? 0),
      name: b.name ?? `${Math.round(b.maxGirthCm ?? b.maxSumCm ?? 0)} cm`,
    })).sort((a: {max:number;price:number;name:string}, b: {max:number;price:number;name:string}) => a.max - b.max);
    const band = norm.find((b: {max:number;price:number;name:string}) => girthCm <= b.max) || norm.at(-1)!;
    return { band: band.name, price: band.price };
  }
...
export function priceSingle(
  ...
  const inkRate = (s.inkCostPerSqm ?? s.inkElecPerSqm ?? 0); let ink = areaSqm * inkRate
  ...
  if (input.mode === 'SolidColourCutVinyl') {
    ...
    vinylCostItems.push({ media: mediaItem.name, lm: +(lm).toFixed(3) as unknown as number, pricePerLm: mediaItem.pricePerLm, cost: +((lm) * mediaItem.pricePerLm).toFixed(2) })
  ...
  if (input.mode === 'PrintedVinylOnSubstrate' || input.mode === 'SubstrateOnly') {
    ...
    // Double-sided affects print cost usually; to preserve existing behaviour we keep substrate as chargedSheets only
